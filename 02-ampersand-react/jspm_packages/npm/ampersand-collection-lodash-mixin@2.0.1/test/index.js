...
var collection;
var Model = AmpersandState.extend({props: {
    id: 'number',
    foo: 'string',
    bar: 'string'
  }});
var Collection = AmpersandCollection.extend(AmpersandLodashMixins, {model: Model});
var methods = ['forEach', 'each', 'map', 'reduce', 'reduceRight', 'find', 'filter', 'reject', 'every', 'some', 'includes', 'invoke', 'max', 'min', 'size', 'first', 'take', 'initial', 'rest', 'drop', 'last', 'without', 'difference', 'indexOf', 'shuffle', 'lastIndexOf', 'isEmpty', 'sample', 'partition', 'groupBy', 'countBy', 'sortBy', 'indexBy'];
...
test('`where` and `findWhere` methods should filter a collection based on a given attributes', function(t) {
  ...
  var whereSingleAttr = collection.where({foo: 'baz'});
  ...
  var findWhereSingleAttr = collection.findWhere({foo: 'baz'});
...
test('`first` method should return first model', function(t) {
  ...
  t.deepEqual(first, {
    ...
    foo: 'baz',
    bar: 'qux'
  }, 'verify that returned attribute values are correct');
