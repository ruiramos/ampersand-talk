...
function getView(bindings, model) {
  ...
  var view = new View({model: model || new Model()});
...
test('event behavior with over-ridden `render` & `remove` fns', function(t) {
  ...
  var removeCount = 0;
  t.plan(7);
  var ChildView = AmpersandView.extend({render: function() {
      AmpersandView.prototype.render.call(this);
      t.ok(true, 'child view render called');
    }});
  var GrandChildView = AmpersandView.extend({render: function() {
...
test('registerSubview', function(t) {
  ...
  var main = new View({el: document.createElement('div')});
  main.render();
...
test('listen to and run', function(t) {
  t.plan(1);
  var model = new Model({props: {name: 'string'}});
  var View = AmpersandView.extend({
    initialize: function() {
      this.model = model;
...
test('text bindings', function(t) {
  var view = getView({bindings: {'model.name': 'span'}});
...
test('src bindings', function(t) {
  var view = getView({bindings: {'model.url': {
        type: 'attribute',
        name: 'src',
        selector: 'img'
      }}});
...
test('nested binding definitions', function(t) {
  ...
  var View = AmpersandView.extend({
    ...
    bindings: {'model.active': [{
        ...
        name: 'data-something',
        selector: 'div'
      }, {selector: 'div'}, {type: 'booleanClass'}]}
  });
...
test('renderAndBind with no model', function(t) {
  var View = AmpersandView.extend({template: '<li><span></span><img></li>'});
  var view = new View();
  t.ok(view.renderWithTemplate());
  t.end();
});
test('queryByHook', function(t) {
  var View = AmpersandView.extend({template: '<li data-hook="list-item"><span data-hook="username"></span><img data-hook="user-avatar"></li>'});
  var view = new View();
...
test('queryAllByHook', function(t) {
  var View = AmpersandView.extend({template: '<li data-hook="list-item"><span data-hook="username info"></span><img data-hook="user-avatar info"></li>'});
  var view = new View();
  view.renderWithTemplate();
...
test('throw on multiple root elements', function(t) {
  var View = AmpersandView.extend({template: '<li></li><div></div>'});
  var view = new View();
...
test('Multi-inheritance of state properties works too', function(t) {
  ...
  var SecondView = View.extend({props: {otherThing: 'boolean'}});
...
test('declarative subViews basics', function(t) {
  var Sub = AmpersandView.extend({template: '<span></span>'});
  var View = AmpersandView.extend({
...
test('make sure subviews dont fire until their `waitFor` is done', function(t) {
  var Sub = AmpersandView.extend({template: '<span>yes</span>'});
  var View = AmpersandView.extend({
...
test('make sure template can return a dom node', function(t) {
  var Sub = AmpersandView.extend({template: function() {
...
test('template can be passed as viewOption', function(t) {
  t.plan(1);
  var View = AmpersandView.extend({autoRender: true});
  var view = new View({template: '<span></span>'});
